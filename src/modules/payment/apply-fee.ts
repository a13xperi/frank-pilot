        ...
        await query(
          `UPDATE applications
              SET status = 'submitted',
                  ...
                  -- voice/DTMF fee-paid path is consistent: this block only runs
                  -- when consent is on file (hasValidAuthorization above), so the
                  -- app IS authorized — record it. Without this,
                  -- screening_authorization_at stays null on voice apps and the
                  -- consented flag + adverse-action code read a false negative.
