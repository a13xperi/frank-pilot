...
describe("GET /properties", () => {
  ...
  it("returns 200 with properties array and total count", async () => {
    const sampleProp = { id: "prop-001", name: "Sunrise Apts", city: "Atlanta", state: "GA" };
    mockAuthQuery(seniorManager);
    mockList.mockResolvedValue([sampleProp]);
    const res = await request(app)
      .get("/properties")
  ...
  it("returns 500 when service throws", async () => {
    ...
    mockList.mockRejectedValue(new Error("DB connection lost"));
    const res = await request(app)
      .get("/properties")
